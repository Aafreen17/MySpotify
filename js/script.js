...
async function main() {
  ...
  previous.addEventListener("click", () => {
    currentSong.pause();
    play.src = "img/play.svg";
    const currentSongFilename = decodeURIComponent(
      currentSong.src.split("/").pop()
    );
    let index = songs.findIndex((song) =>
      decodeURIComponent(song).endsWith(currentSongFilename)
    );
    if (index - 1 >= 0) {
